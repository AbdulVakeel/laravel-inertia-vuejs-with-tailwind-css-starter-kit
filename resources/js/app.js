...
createInertiaApp({
    resolve: (name) => {
        ...
        page.then((module) => {
            module.default.layout = module.default.layout != false
    ...
    setup({ el, App, props, plugin }) {
        ...
        app.config.globalProperties.ago = (val) =>
            val ? moment(val).fromNow() : '';

        app.config.globalProperties.formatNumber = window.formatNumber = (
            val,
            precision = 2
        ) => val ? parseFloat(val).toFixed(precision) : '';

        app.config.globalProperties.randomKey = window.randomKey = (
            val = null
        ) => val ?? Date.now().toString() + Math.random();
