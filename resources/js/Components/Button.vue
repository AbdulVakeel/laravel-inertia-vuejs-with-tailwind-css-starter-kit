...
const props = defineProps({
	as: { type: String, default: 'button' }, // Default type button
	href: { type: String, default: '#' }, // Default href
...
const processingClasses = computed(() => (props.processing ? 'opacity-25' : ''));
...
<template>
	<!-- Button Element -->
	<button v-if="as === 'button'" :class="[buttonClass, sizeClass, processingClasses]">
		<slot />
	</button>

	<a v-else-if="as === 'a'" :href="href" :class="[buttonClass, sizeClass, processingClasses]">
		<slot />
	</a>

	<!-- Inertia.js Link -->
	<Link v-else :href="href" :class="[buttonClass, sizeClass, processingClasses]">
		<slot />
	</Link>
</template>
