...
const {adminNavigations} = useNavigations(isSidebarOpen.value);
...
<template>
	<div class="bg-dark text-dark">
		...
		<div
			...
			<SideNavbar :key="$page.url" menu-type="admin" />
		</div>

		<!-- Static sidebar for mobile -->
		<transition
			v-if="mobileMenu"
			enter-from-class="translate-x-[150%] opacity-0 duration-500"
			leave-to-class="translate-x-[150%] opacity-0 duration-500"
			enter-active-class="transition duration-300"
			leave-active-class="transition duration-300">
			<div :class="['flex flex-col fixed inset-y-0 z-50 md:hidden w-full ', !mobileMenu ? 'relative ' : '']">
				<MobileSideNavbar :open="mobileMenu"  menu-type="admin" :key="Date.now().toString + Math.random()"
													@close="toggleMobileMenu"/>
			</div>
		</transition>

		<div
			:class="['flex flex-col flex-1 z-10 transition-all duration-300 delay-150',
